...
def load_gui(self):
    ...
    self.lstList1.bind('<<ListboxSelect>>',lambda event: stFunc.onSelect(self,event))
    ...
    self.btn_submit = tk.Button(self.master,width=12,height=2,text='Submit',command=lambda: stFunc.addToList(self))
    self.btn_submit.grid(row=10,column=3,padx=(25,0),pady=(5,10),sticky=E)
    self.btn_delete = tk.Button(self.master,width=12,height=2,text='Delete',command=lambda: stFunc.onDelete(self))
    self.btn_delete.grid(row=10,column=4,padx=(15,0),pady=(5,10),sticky=E)

    stFunc.create_db(self)
    stFunc.onRefresh(self)
